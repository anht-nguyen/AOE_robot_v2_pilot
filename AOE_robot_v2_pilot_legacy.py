...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\anhtn\\OneDrive - PennO365\\Documents\\GitHub\\AOE_robot_v2_pilot\\AOE_robot_v2_pilot_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    ITI_marker = visual.BaseVisualStim(win=win, name="ITI_marker")
    ...
    executionCircle = visual.ShapeStim(
        ...
        opacity=None, depth=-1.0, interpolate=True)
    # This is generated by writeInitCode
    executionMarker = visual.BaseVisualStim(win=win, name="executionMarker")
    ...
    loopLoader = data.TrialHandler(nReps=1.0, method='sequential', 
        ...
        trialList=data.importConditions('trial_files/AOE_final_randomized_trials.xlsx'),
    ...
    for thisLoopBlock in loopBlock:
        ...
        for thisLoopTrial in loopTrial:
            ...
            itiLen = round(random.uniform(3, 3.5), 3)
            itiVal = 3000
            print('-*'*100)
            print('itiVal: ', itiVal)
            ...
            # keep track of which components have finished
            interTrialIntervalComponents = [skipITI, ITI_marker]
            ...
            while continueRoutine:
                ...
                # if ITI_marker is starting this frame...
                if ITI_marker.status == NOT_STARTED and t >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    ITI_marker.frameNStart = frameN  # exact frame index
                    ITI_marker.tStart = t  # local t and not account for scr refresh
                    ITI_marker.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(ITI_marker, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.addData('ITI_marker.started', t)
                    # update status
                    ITI_marker.status = STARTED
                    ITI_marker.status = STARTED
                    ...
                    ITI_marker.start_sent = True
                
                # if ITI_marker is stopping this frame...
                if ITI_marker.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > ITI_marker.tStartRefresh + itiLen-frameTolerance:
                        # keep track of stop time/frame for later
                        ITI_marker.tStop = t  # not accounting for scr refresh
                        ITI_marker.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.addData('ITI_marker.stopped', t)
                        # update status
                        ITI_marker.status = FINISHED
                        ITI_marker.status = FINISHED
            ...
            fixationLen = round(random.uniform(1.5, 2),3) 
            #fixationLen = 0.75
            #fixationLen = 1.5
            fixationVal = 1000
            ...
            while continueRoutine:
                ...
                if stimMarker.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > stimMarker.tStartRefresh + 10-frameTolerance:
                        # keep track of stop time/frame for later
            ...
            # This is generated by the writeRoutineStartCode
            # keep track of which components have finished
            executionPhaseComponents = [executionCircle, executionMarker]
            ...
            while continueRoutine and routineTimer.getTime() < 5.0:
                ...
                if executionCircle.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > executionCircle.tStartRefresh + 5-frameTolerance:
                        # keep track of stop time/frame for later
                ...
                # if executionMarker is starting this frame...
                if executionMarker.status == NOT_STARTED and t >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    executionMarker.frameNStart = frameN  # exact frame index
                    executionMarker.tStart = t  # local t and not account for scr refresh
                    executionMarker.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(executionMarker, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.addData('executionMarker.started', t)
                    # update status
                    executionMarker.status = STARTED
                    executionMarker.status = STARTED
                    delta_time = tThisFlip-t  # Adding the extra time between now and the next screen flip
                    cortex_obj.inject_marker(value=str(exeMarkerVal), label=stimList[counterStim], delta_time=delta_time)
                    executionMarker.start_sent = True
                
                # if executionMarker is stopping this frame...
                if executionMarker.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > executionMarker.tStartRefresh + 5-frameTolerance:
                        # keep track of stop time/frame for later
                        executionMarker.tStop = t  # not accounting for scr refresh
                        executionMarker.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.addData('executionMarker.stopped', t)
                        # update status
                        executionMarker.status = FINISHED
                        executionMarker.status = FINISHED
            ...
            # Run 'End Routine' code from codeExe
            counterStim += 1
            # This is generated by the writeRoutineEndCode
            # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
            if routineForceEnded:
